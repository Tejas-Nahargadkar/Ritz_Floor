...
import { ReactComponent as ArrowLeft } from "../asset/chevron-left-solid.svg";
import { ReactComponent as ArrowRight } from "../asset/chevron-right-solid.svg";
import { ReactComponent as Circle } from "../asset/circle-solid.svg";
...
class Carousel extends React.Component {
  ...
  componentDidMount() {
    // gets index of active card from imported data and sets state
  ...
  getLength() {
    // calculating length of imported array
    let length = 0;
  ...
  activateNextSlide() {
    let rightIndex = 0;
    if (this.state.rightCard + 1 === this.state.arrayLength) {
      // checking for out of bounds
      ...
      <div className="carousel-container">
        <div className="cards">
          <div className="card leftCard">
            <img
              className="avatars"
              src={items[this.state.leftCard].imgSrc}
              alt="avatar"
            ></img>
            <div className="text">
          ...
          <div className="card active">
            <img
              className="avatars"
              src={items[this.state.activeIndex].imgSrc}
              alt="avatar"
            ></img>
            <span>
              <div className="text">
                ...
                <i className="position">
                  {items[this.state.activeIndex].position}
                </i>
                <p className="testimony">
                  {items[this.state.activeIndex].quote}
                </p>
              </div>
              <div className="arrow-container">
                <ArrowLeft
                  ...
                  onClick={this.activatePreviousSlide.bind(this)}
                ></ArrowLeft>
                <ArrowRight
                  className="arrow arrow-right"
                  fill="hsla(221, 0%, 49%, 0.98)"
                  onClick={this.activateNextSlide.bind(this)}
                ></ArrowRight>
              </div>
            </span>
          </div>
          <div className="card rightCard">
            <img
              className="avatars"
              src={items[this.state.rightCard].imgSrc}
              alt="avatar"
            ></img>
            <div className="text">
        ...
        <div className="tracker">{trackerArray}</div>
      </div>
    );
  }
}
